...
## 提示2：
D:\1SRO_DevKit - Bancroft 此目录是丝路传说CSRO的服务端和客户端插件源码，其中就有 “移到到队友位置” 的源文件，在游戏中复现也是可实现队友之间的相互传送，但问题是，这个源文件的功能不齐全，在队友传送的过程中缺少了很多限制，例如：限制的副本也能传送、限制的职业状态下也能传送、还有传送功能不完整，传送的界面UI与ISRO的传送界面UI不同，所以在参照这个源文件的时候要特别注意。


## 任务执行提示

1. 如果任务来自于 github issues, 那么在对应的issues中以 comment 的方式给我回复。
2. 当在 github issues 回复时， 源码和文档的引用应当是超链接，这样我在github中可以方便点击查看相关文档。
